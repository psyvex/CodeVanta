...
The repository has the production-oriented polyglot foundation in place: a Rust analysis engine and CLI covering JavaScript/TypeScript and Python (deterministic AST analyzers plus local-model review), a GitHub pull-request mining and human-in-the-loop labeling pipeline, a Python dataset/training/evaluation pipeline (proven end-to-end against a tiny offline model), model manifests for the javascript-typescript and python specializations, shared JSON schemas, and CI across all three stacks. See `docs/model-pipeline.md` for the full pipeline and what still needs a GPU and licensed data versus what runs today.
